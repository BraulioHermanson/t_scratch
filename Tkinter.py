...
#https://www.canva.com/colors/color-palettes/here-at-home/
...
mensagem = tk.Label(text ="Sistema de Busca de Cotação de Moedas",foreground='white',background='#647C90',width=50,height=10)
mensagem.grid(row=0, column=0, columnspan=2,sticky="EW")

mensagem2 = tk.Label(text="Selecionar moeda desejada",fg='white',bg='#4E4F50',width=50,height=10)
...
def buscar_cotacao():
    ...
    mensagem_cotacao.grid(row=4, column=0)
    if cotacao_moeda:
        mensagem_cotacao["text"] = f"Cotação do {moeda_preenchida} é de {cotacao_moeda} reais."



botao = tk.Button(text="Bucar Cotação",command=buscar_cotacao)
